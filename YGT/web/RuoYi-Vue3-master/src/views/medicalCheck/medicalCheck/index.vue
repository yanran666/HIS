...
<script setup name="MedicalCheck">
import {
  listMedicalCheck,
  getMedicalCheck,
  delMedicalCheck,
  addMedicalCheck,
  updateMedicalCheck,
} from "@/api/medicalCheck/medicalCheck";
...
function getList() {
  loading.value = true;
  listMedicalCheck(queryParams.value).then((response) => {
    medicalCheckList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("medicalCheckRef");
}
...
function handleUpdate(row) {
  ...
  getMedicalCheck(_medicalRecordId).then((response) => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["medicalCheckRef"].validate((valid) => {
    if (valid) {
      if (form.value.medicalRecordId != null) {
        updateMedicalCheck(form.value).then((response) => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addMedicalCheck(form.value).then((response) => {
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除病历记录编号为"' + _medicalRecordIds + '"的数据项？')
    .then(function () {
      return delMedicalCheck(_medicalRecordIds);
    })
    .then(() => {
      getList();
...
function handleExport() {
  proxy.download(
    "medicalCheck/medicalCheck/export",
    {
      ...queryParams.value,
    },
    `medicalCheck_${new Date().getTime()}.xlsx`
  );
}
